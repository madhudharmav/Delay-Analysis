...
shinyServer(function(input, output,session) {
 ...
 pre_pro<-reactive({
   ...
   # df_real_raw<-read.csv(paste(path,"widget_all.csv",sep="/") ,sep=",", stringsAsFactors = FALSE)
 ...
 output$table1 <- renderTable({
   ...
   for(i1 in classes){
     ...
     no_delay2a[i2]<-as.integer(df_week1[cluster ==input$e2 & class == i1 & timewin == 30, .N])
     no_delay2b[i2]<-as.integer(df_week1[cluster == input$e2 & class == i1 & timewin ==120, .N])
     no_delay2c[i2]<-as.integer(df_week2[cluster ==input$e2 & class == i1 & timewin == 30, .N])
     no_delay2d[i2]<-as.integer(df_week2[cluster == input$e2 & class == i1 & timewin ==120, .N])
   ...
   op_table1[which(op_table1==NaN)]<-"No orders"
 ...
 output$table2 <- renderTable({
   ...
   for(i3 in timeslots){
     ...
     no_delay4a[i2]<-as.integer(df_week1[cluster == input$e2  & timeslot == i3 & timewin ==30, .N])
     no_delay4b[i2]<-as.integer(df_week1[cluster == input$e2  & timeslot == i3 & timewin ==120, .N])
